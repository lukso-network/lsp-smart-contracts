...
- avoiding the need to be familiar with different programming languages of unrelated areas. It is not difficult to write simple files in unfamiliar programming languages but it could be somewhat cumbersome to validate the syntax of the source code as it could require a) to search for online REPL tools or validators that could easily be not ideal (sometimes they are intentionally limited) or b) manually preparing your local environment to use a specific language (can be a lot more difficult);
...
- `documentation` - can be absent, `null`, string value or an array of other primitive type objects like `Int`s and `String`s combined (e.g. `[1, "2", 3.0]`).
