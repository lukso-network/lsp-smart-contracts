...
import "./ILSP20CallVerification.sol";

// errors
import "./LSP20Errors.sol";

/**
 * @title Implementation of the reverse verification according to LSP20CallVerification
...
abstract contract LSP20CallVerification {
        ...
        if (msg.sender != logicVerifier) {
            (bool success, bytes memory returnedData) = logicVerifier.call(
                abi.encodeWithSelector(
                    ILSP20CallVerification.lsp20VerifyCall.selector,
                    msg.sender,
                    msg.value,
                    msg.data
                )
            ...
            if (!success) {
                ...
                if (returnedData.length > 0) {
                    ...
                    revert LSP20CallingVerifierFailed(false);
                }
            }
            ...
            if (bytes3(magicValue) != bytes3(ILSP20CallVerification.lsp20VerifyCall.selector))
    ...
    function _verifyCallResult(address logicVerifier, bytes memory callResult) internal virtual {
        (bool success, bytes memory returnedData) = logicVerifier.call(
            abi.encodeWithSelector(
                ILSP20CallVerification.lsp20VerifyCallResult.selector,
        ...
        if (!success) {
            ...
            if (returnedData.length > 0) {
                ...
                revert LSP20CallingVerifierFailed(true);
            }
        }
