...
library ERC725Utils {
    ...
    function getDataSingle(ERC725Y _account, bytes32 _key) public view returns (bytes memory) {
    ...
    function getDataSingle(ERC725YInit _account, bytes32 _key) public view returns (bytes memory) {
        bytes32[] memory keys = new bytes32[](1);
        keys[0] = _key;
        bytes memory fetchResult = _account.getData(keys)[0];
        return fetchResult;
