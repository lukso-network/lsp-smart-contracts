 ...
 *
 * It contains a storage of nonces for signer addresses across various channel IDs for a specific account, enabling these signers to submit signed transactions that order-independant.
 ...
 */
abstract contract LSP25MultiChannelNonceSingleton {
    using ECDSA for *;

    // Mapping of account => signer -> channelId -> nonce in channel
    mapping(address => mapping(address => mapping(uint256 => uint256)))
        internal _nonceStore;
     ...
     * ```
     *
     * @param account The account to read the nonce for.
    ...
    function _getNonce(
        ...
        return
            (uint256(channelId) << 128) | _nonceStore[account][from][channelId];
    }
     ...
     *
     * @param target The address of the contract you are executing the payloads on.
    ...
    function _isValidNonce(
        ...
        return (idx & mask) == _nonceStore[account][from][idx >> 128];
    }
}
