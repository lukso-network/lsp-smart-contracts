...
const UniversalReceiverAddressStoreInit = artifacts.require(
...
contract("Receivers with Proxy (using Truffle)", (accounts) => {
  ...
  let lsp3Account, uniAddressStoreMaster, uniAddressStoreProxy;
  ...
  context("UniversalReceiverAddressStore", async () => {
    it("should be cheaper to deploy via proxy", async () => {
      ...
      let contractDeploymentCost =
        await UniversalReceiverAddressStore.new.estimateGas(
          lsp3Account.address
        );

      // Deploying via Proxy
      let masterContract = await UniversalReceiverAddressStoreInit.new(
        lsp3Account.address
      );
      ...
      let proxy = await UniversalReceiverAddressStoreInit.at(
        tx.contractAddress
      );

      let proxyDeploymentCost = tx.gasUsed;
      let initializeCost = await proxy.initialize.estimateGas(
        lsp3Account.address
      );
    ...
    it("Should call the `initialize(...)` function", async () => {
      ...
      assert.notEqual(
        newAccount,
        currentAccount,
        "Associated account has not changed"
      );
      assert.equal(
        newAccount,
        lsp3Account.address,
        "Associated account should be `lsp3Account.address`"
      );
    });

    it("should not allow to initialize twice", async () => {
      await truffleAssert.fails(
        uniAddressStoreProxy.initialize(
