...
contract("KeyManagerHelper", async (accounts) => {
  let keyManagerHelper, erc725Account;
  ...
  before(async () => {
    ...
    await erc725Account.setData(
      KEY_PERMISSIONS + owner.substr(2),
      ALL_PERMISSIONS,
      { from: owner }
    );
    await erc725Account.setData(
  ...
  context("Reading ERC725's account storage", async () => {
    it("_getAllowedAddresses(...) - Should return list of owner's allowed addresses", async () => {
      let bytesResult = await keyManagerHelper.getAllowedAddresses(owner, {
        from: owner,
      });
      let allowedOwnerAddresses = web3.eth.abi.decodeParameter(
        "address[]",
        bytesResult
      );

      assert.deepEqual(allowedOwnerAddresses, allowedAddresses);
    });
    ...
    it("_getAllowedFunctions(...) - Should return list of owner's allowed functions", async () => {
      ...
      let allowedOwnerFunctions = web3.eth.abi.decodeParameter(
        "bytes4[]",
        bytesResult
      );
      ...
      let decodedResultFromAccount = web3.eth.abi.decodeParameter(
        "bytes4[]",
        resultFromAccount
      );
      ...
      assert.equal(
        bytesResult,
        resultFromAccount,
        "KeyManager and ERC725 account do not return the same result"
      );
    });
...
contract("KeyManager", async (accounts) => {
  ...
  before(async () => {
    ...
    await erc725Account.setData(
      ...
      KEY_ALLOWEDFUNCTIONS + app.substr(2),
      web3.eth.abi.encodeParameter("bytes4[]", [
        web3.eth.abi.encodeFunctionSignature("setName(string)"),
      ])
    );

    // user permissions
    let userPermissions = web3.utils.toHex(
      PERMISSION_SETDATA + PERMISSION_CALL
    );
    await erc725Account.setData(
      KEY_PERMISSIONS + user.substr(2),
      userPermissions,
      { from: owner }
    );

    // externalApp permissions
    let externalAppPermissions = web3.utils.toHex(
      PERMISSION_SETDATA + PERMISSION_CALL
    );
    await erc725Account.setData(
      KEY_PERMISSIONS + externalApp.substr(2),
    ...
    await erc725Account.setData(
      ...
      web3.utils.toHex(
        PERMISSION_SETDATA + PERMISSION_CALL + PERMISSION_TRANSFERVALUE
      ),
      { from: owner }
    );
  ...
  it("ensures owner is still erc725Account's admin (=all permissions)", async () => {
    let permissions = await erc725Account.getData(
      KEY_PERMISSIONS + owner.substr(2)
    );
    assert.equal(
      permissions,
  ...
  context("> testing permissions: CHANGEKEYS, SETDATA", async () => {
    it("Owner should be allowed to change keys", async () => {
      let payload = erc725Account.contract.methods
        .setData(KEY_PERMISSIONS + app.substr(2), PERMISSION_SETDATA)
        .encodeABI();
    ...
    it("App should not be allowed to change keys", async () => {
      ...
      let dangerousPayload = erc725Account.contract.methods
        .setData(KEY_PERMISSIONS + app.substr(2), ALL_PERMISSIONS)
        .encodeABI();
  ...
  context("> testing permission: TRANSFERVALUE", async () => {
    it("Owner should be allowed to transfer ethers to app", async () => {
      ...
      let callResult = await keyManager.execute.call(transferPayload, {
        from: owner,
      });
      assert.isTrue(
        callResult,
        "Low Level Call failed (=returned `false`) for: KeyManager > ERC725Account > TargetContract"
      );
      ...
      assert.isBelow(
        ...
        "ERC725 Account's balance should have decreased"
      );

      let newAppBalance = await web3.eth.getBalance(app);
      assert.isAbove(
        parseInt(newAppBalance),
    ...
    it("App should not be allowed to transfer ethers", async () => {
      ...
      assert.equal(
        ...
        newAccountBalance,
        "ERC725 Account's balance should have remained the same"
      );
      assert.equal(
        initialUserBalance,
        newUserBalance,
        "User's balance should have remained the same"
      );
    });
  });
  ...
  context("> testing permissions: ALLOWEDADDRESSES", async () => {
    it("All addresses whitelisted = Owner should be allowed to interact with any address", async () => {
      ...
      let secondPayload = erc725Account.contract.methods
        .execute(
          OPERATION_CALL,
          web3.utils.toChecksumAddress(
    ...
    it("App should be allowed to interact with `TargetContract`", async () => {
      let payload = erc725Account.contract.methods
        .execute(OPERATION_CALL, targetContract.address, 0, EMPTY_PAYLOAD)
        .encodeABI();
    ...
    it("App should be allowed to interact with `user`", async () => {
      let payload = erc725Account.contract.methods
        .execute(OPERATION_CALL, user, 0, EMPTY_PAYLOAD)
        .encodeABI();
  ...
  context("> testing: ALL ADDRESSES + FUNCTIONS whitelisted", async () => {
    it("Should pass if no addresses / functions are stored for a user", async () => {
      ...
      let callResult = await keyManager.execute.call(executePayload, {
        from: user,
      });
      assert.isTrue(
        callResult,
        "Low Level Call failed (=returned `false`) for: KeyManager > ERC725Account > RandomAddress"
      );
  ...
  context("> testing external contract's state change", async () => {
    it("Owner should be allowed to set `name` variable in simple contract", async () => {
      ...
      assert.notEqual(
        result,
        initialName,
        "name variable in TargetContract should have changed"
      );
      assert.equal(
        result,
        newName,
  ...
  context("> testing other revert causes", async () => {
    it("Should revert because of wrong operation type", async () => {
      let payload = erc725Account.contract.methods
        .execute(5648941657, targetContract.address, 0, "0x")
        .encodeABI();
    ...
    it("Should revert because calling an unexisting function in ERC725", async () => {
      await truffleAssert.fails(
        keyManager.execute("0xbad0000000000000000000000000bad", {
          from: owner,
        }),
        // truffleAssert.ErrorType.REVERT,
...
contract("KeyManager", async (accounts) => {
    ...
    before(async () => {
      ...
      assert.equal(
        endResult,
        newName,
        "Name on TargetContract has not changed"
      );
    });
