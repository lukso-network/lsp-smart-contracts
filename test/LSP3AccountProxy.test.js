...
contract("LSP3Account via EIP1167 Proxy + initializer (using Truffle)", async (accounts) => {
    ...
    context("> Universal Receiver", async () => {
        ...
        xit("Call account and check for 'ReceivedERC777' event in external account", async () => {
            ...
            await newProxy.setData(UNIVERSALRECEIVER_KEY, externalUniversalReceiver.address, { from: owner })
        ...
        xit("Mint ERC777 and LSP4 to LSP3 account", async () => {
            ...
            await digitalCertificate.mint(proxy.address, '50', { from: tokenOwner })
...
contract.skip("> LSP3Account via EIP1167 Proxy + initializer (using Web3)", async (accounts) => {
    ...
    context("> Testing storage", async () => {
        ...
        it("Store a long URL as bytes item 6", async () => {
            ...
            let key = web3.utils.numberToHex(count++)
            let value = web3.utils.utf8ToHex(url)
            await proxy.methods.setData(key, value).send({ from: owner, gas: 300_000 })
        ...
        it("Store multiple 32 bytes item 9-11", async () => {
            ...
            await proxy.methods.setDataMultiple(keys, values).send({ from: owner, gas: 300_000 })
            // console.log(await proxy.getDataMultiple(keys))
            assert.deepEqual(await proxy.methods.getDataMultiple(keys).call(), values)
        })
    ...
    context("> Interactions with Accounts contracts", async () => {
        ...
        beforeEach(async () => {
            LSP3Proxy = await deployLSP3Proxy(LSP3AccountInit, lsp3Account.address, owner)
            await LSP3Proxy.methods.initialize(owner).send({ from: owner, gas: 300_000 })
        })
        
        it("Upgrade ownership correctly", async () => {
            await LSP3Proxy.methods.transferOwnership(newOwner).send({ from: owner, gas: 300_000 })
        ...
        xit("Allows owner to execute calls", async () => {
            ...
            await LSP3Proxy.methods.execute(OPERATION_CALL, dest, amount, "0x0").send({
                from: owner,
                gas: 3_000_000
            });
            ...
            assert.isTrue(
                new BN(destBalance).add(amount)
                    .eq(new BN(finalBalance))
            )
        ...
        it("Allows owner to execute create", async () => {
            ...
            let receipt = await LSP3Proxy.methods.execute(OPERATION_CREATE, '0x0000000000000000000000000000000000000000', '0', "")
                .send({
                    from: owner,
                    gas: 3_000_000
                })
            ...
            expect(receipt.events)  
                .to.have.property('ContractCreated')
