...
export const shouldBehaveLikeLSP8Enumerable = (
  ...
  describe('when no minted tokens', () => {
    it('should not get token', async () => {
      ...
      expect(await context.lsp8Enumerable.tokenAt(tokenSupply)).toEqual(ethers.utils.hexZeroPad(0, 32));
    });
  });
  ...
  describe('when minted tokens', () => {
    ...
    it('should access by index', async () => {
      ...
      expect(await context.lsp8Enumerable.totalSupply()).toEqual(tokenSupply.add(1));
      expect(await context.lsp8Enumerable.tokenAt(tokenSupply)).toEqual(ethers.utils.hexlify(tokenId));
    });
    ...
    it('should not access by index after removed', async () => {
      ...
      expect(await context.lsp8Enumerable.totalSupply()).toEqual(tokenSupply.add(1));
      expect(await context.lsp8Enumerable.tokenAt(tokenSupply)).toEqual(ethers.utils.hexlify(anotherTokenId));
      expect(await context.lsp8Enumerable.tokenAt(tokenSupply.add(1))).toEqual(ethers.utils.hexZeroPad(0, 32));
    });
    ...
    it('should access by index after removed', async () => {
      ...
      await context.lsp8Enumerable.burn(tokenId);
      await context.lsp8Enumerable.mint(context.accounts.tokenReceiver.address, tokenId);
      expect(await context.lsp8Enumerable.totalSupply()).toEqual(tokenSupply.add(2));
      expect(await context.lsp8Enumerable.tokenAt(tokenSupply)).toEqual(ethers.utils.hexlify(anotherTokenId));
      expect(await context.lsp8Enumerable.tokenAt(tokenSupply.add(1))).toEqual(ethers.utils.hexlify(tokenId));
    });
  });
};
