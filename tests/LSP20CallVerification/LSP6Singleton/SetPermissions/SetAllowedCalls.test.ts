...
import { setupKeyManager, setupKeyManagerSingleton } from '../../../utils/fixtures';

// helpers
import { combineAllowedCalls } from '../../../utils/helpers';

export const shouldBehaveLikeSetAllowedCalls = (
  buildContext: () => Promise<LSP6SingletonTestContext>,
) => {
  ...
  describe('deleting AllowedCalls', () => {
    ...
    before(async () => {
      ...
      await setupKeyManagerSingleton(context, permissionKeys, permissionValues);
    });

    describe('when caller has ADD permission', () => {
      describe('when controller / beneficiary has some permissions', () => {
      ...
      describe('when controller / beneficiary has no permissions', () => {
        it('should pass and set the list of allowed calls for an address', async () => {
          const dataKey =
            ERC725YDataKeys.LSP6['AddressPermissions:AllowedCalls'] +
            beneficiaryNoPermissions.address.substring(2);
          const dataValue = '0x';

          await context.universalProfile.connect(canOnlyAddController).setData(dataKey, dataValue);
    ...
    describe('when caller has EDIT permission', () => {
      describe('when controller / beneficiary has some permissions', () => {
      ...
      describe('when controller / beneficiary has no permissions', () => {
        it("should revert with error `NotAuthorised('EDITPERMISSIONS')` when trying to clear the list of allowed calls for an address", async () => {
          const dataKey =
            ERC725YDataKeys.LSP6['AddressPermissions:AllowedCalls'] +
            beneficiaryNoPermissions.address.substring(2);
          const dataValue = '0x';
          ...
          await expect(
            context.universalProfile.connect(canOnlyEditPermissions).setData(dataKey, dataValue),
          )
            .to.be.revertedWithCustomError(context.keyManager, 'NotAuthorised')
  ...
  describe('setting Allowed Calls -> Addresses', () => {
    ...
    describe('when caller has permission ADDCONTROLLER', () => {
      ...
      describe('when beneficiary (= controller) had 00 bytes set initially as allowed calls (e.g: allowed calls disabled)', () => {
        it('should fail with NotAuthorised -> beneficiary had 32 x 0 bytes set initially as allowed calls', async () => {
          const key =
            ERC725YDataKeys.LSP6['AddressPermissions:AllowedCalls'] +
        ...
        it('should fail with NotAuthorised -> beneficiary had 40 x 0 bytes set initially as allowed calls', async () => {
          const key =
            ERC725YDataKeys.LSP6['AddressPermissions:AllowedCalls'] +
      ...
      describe('when setting an invalid bytes28[CompactBytesArray] for a new beneficiary', () => {
        ...
        it('should revert with error when value = invalid bytes28[CompactBytesArray] (not enough bytes, missing the first length bytes)', async () => {
          ...
          const value = '0xffffffffcafecafecafecafecafecafecafecafecafecafeffffffff';
    ...
    describe('when caller has permission ADDCONTROLLER', () => {
      ...
      describe('when setting an invalid bytes28[CompactBytesArray] for a new beneficiary', () => {
        ...
        it('should fail when setting an invalid bytes28[CompactBytesArray] (not enough bytes, missing first length byte)', async () => {
          ...
          const value = '0xffffffffffffffffffffffffffffffffffffffffffffffffcafecafe';
  ...
  describe('setting Allowed Calls -> Standards', () => {
    ...
    describe('when caller has ADDCONTROLLER', () => {
      describe('when controller / beneficiary has some allowed calls set', () => {
