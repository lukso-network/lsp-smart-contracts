...
export const shouldBehaveLikePermissionSetData = (
  ...
  describe('when caller is an EOA', () => {
    ...
    before(async () => {
      ...
      await setupKeyManagerSingleton(context, permissionsKeys, permissionsValues);
    });
  ...
  describe('when caller is another UniversalProfile (with a KeyManager attached as owner)', () => {
    ...
    let aliceContext: LSP6SingletonTestContext;

    // UP being called
    let bob: SignerWithAddress;
    let bobContext: LSP6SingletonTestContext;
    ...
    before(async () => {
      ...
      await setupKeyManagerSingleton(aliceContext, alicePermissionKeys, alicePermissionValues);

      await setupKeyManagerSingleton(bobContext, bobPermissionKeys, bobPermissionValues);
    });
