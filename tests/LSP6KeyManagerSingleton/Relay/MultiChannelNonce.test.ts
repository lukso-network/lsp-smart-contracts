...
export const shouldBehaveLikeMultiChannelNonce = (
  ...
  describe('when calling `getNonce(...)` with a channel ID greater than 2 ** 128', () => {
    it('should revert', async () => {
      ...
      await expect(
        context.keyManager.getNonce(context.universalProfile.address, signer.address, channelId),
  ...
  describe('out of order execution (channel = n)', () => {
    ...
    describe('channel 1', () => {
      ...
      it(`First call > nonce should increment from ${nonces[0]} to ${nonces[0] + 1}`, async () => {
        const nonceBefore = await context.keyManager.callStatic.getNonce(
          context.universalProfile.address,
          signer.address,
          channelId,
        );
      ...
      it(`Second call > nonce should increment from ${nonces[1]} to ${nonces[1] + 1}`, async () => {
        const nonceBefore = await context.keyManager.getNonce(
          context.universalProfile.address,
          signer.address,
          channelId,
        );
