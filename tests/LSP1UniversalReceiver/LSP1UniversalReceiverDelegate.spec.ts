...
describe("UniversalReceiverDelegate contract", () => {
    ...
    beforeAll(async () => {
        ...
        await universalProfile1
            .connect(owner1)
            .setData([ADDRESS.PERMISSIONS + owner1.address.substr(2)], [ALL_PERMISSIONS_SET]);
        ...
        await universalProfile1
            .connect(owner1)
            .setData(
                [ADDRESS.PERMISSIONS + universalReceiverDelegate.address.substr(2)],
                [URDPermissions]
            );
    ...
    describe("Testing Deployement", () => {
        ...
        it("ensures owner is still universalProfile1's admin (=all permissions)", async () => {
            let [permissions] = await universalProfile1.getData([
                ADDRESS.PERMISSIONS + owner1.address.substr(2),
            ]);
        ...
        it("Owner should be allowed to change keys", async () => {
            ...
            let key = ADDRESS.PERMISSIONS + universalReceiverDelegate.address.substr(2);
    ...
    describe("LSP7-DigitalAsset", () => {
        ...
        describe("Minting and Burning Behavior", () => {
            it("Should mint tokenA and Register the Map and the Key in the Array", async () => {
                // Token const
                const tokenAMapAsset = LSP5_ASSET_MAP_HASH + LSP7tokenA.address.substr(2);
                ...
                let [indexAndInterfaceId] = await universalProfile1.connect(owner1).getData([tokenAMapAsset]);
                let [arrayLength] = await universalProfile1.connect(owner1).getData([LSP5_ARRAY_KEY]);
                let [item1Address] = await universalProfile1.connect(owner1).getData([ITEMS_ARRAY_KEY.ITEM1]);
            ...
            it("Should mint TokenB and Register the Map and the Key in the Array and Update the length", async () => {
                // Token const
                const tokenBMapAsset = LSP5_ASSET_MAP_HASH + LSP7tokenB.address.substr(2);
                ...
                let [indexAndInterfaceId] = await universalProfile1.connect(owner1).getData([tokenBMapAsset]);
                let [arrayLength] = await universalProfile1.connect(owner1).getData([LSP5_ARRAY_KEY]);
                let [item2Address] = await universalProfile1.connect(owner1).getData([ITEMS_ARRAY_KEY.ITEM2]);
            ...
            it("Should send more token A and B and don't add or change something", async () => {
                ...
                let [indexAndInterfaceId1] = await universalProfile1.connect(owner1).getData([tokenAMapAsset]);
                let [indexAndInterfaceId2] = await universalProfile1.connect(owner1).getData([tokenBMapAsset]);
                let [arrayLength] = await universalProfile1.connect(owner1).getData([LSP5_ARRAY_KEY]);
                let [item1Address] = await universalProfile1.connect(owner1).getData([ITEMS_ARRAY_KEY.ITEM1]);
                let [item2Address] = await universalProfile1.connect(owner1).getData([ITEMS_ARRAY_KEY.ITEM2]);
                let [item3Address] = await universalProfile1.connect(owner1).getData([ITEMS_ARRAY_KEY.ITEM3]);
            ...
            it("Should mint token C and Register the Map and the key in the Array and update the length", async () => {
                // Token const
                const tokenCMapAsset = LSP5_ASSET_MAP_HASH + LSP7tokenC.address.substr(2);
                ...
                let [indexAndInterfaceId] = await universalProfile1.connect(owner1).getData([tokenCMapAsset]);
                let [arrayLength] = await universalProfile1.connect(owner1).getData([LSP5_ARRAY_KEY]);
                let [item3Address] = await universalProfile1.connect(owner1).getData([ITEMS_ARRAY_KEY.ITEM3]);
            ...
            it("Should Burn token B and update the key in the Array and remove the map", async () => {
                ...
                let [arrayLength] = await universalProfile1.connect(owner1).getData([LSP5_ARRAY_KEY]);
                let [item2Address] = await universalProfile1.connect(owner1).getData([ITEMS_ARRAY_KEY.ITEM2]);
                let [indexAndInterfaceIdForB] = await universalProfile1.connect(owner1).getData([tokenBMapAsset]);
                let [indexAndInterfaceIdForC] = await universalProfile1.connect(owner1).getData([tokenCMapAsset]);
            ...
            it("Should mint Token D and registering the Map and the key in the Array", async () => {
                // Token const
                const tokenDMapAsset = LSP5_ASSET_MAP_HASH + LSP7tokenD.address.substr(2);
                ...
                let [indexAndInterfaceId] = await universalProfile1.connect(owner1).getData([tokenDMapAsset]);
                let [arrayLength] = await universalProfile1.connect(owner1).getData([LSP5_ARRAY_KEY]);
                let [item3Address] = await universalProfile1.connect(owner1).getData([ITEMS_ARRAY_KEY.ITEM3]);
            ...
            it("Should burn Token D and remove the Map and the key in the Array and update the length", async () => {
                ...
                let [arrayLength] = await universalProfile1.connect(owner1).getData([LSP5_ARRAY_KEY]);
                let [item2Address] = await universalProfile1.connect(owner1).getData([ITEMS_ARRAY_KEY.ITEM2]);
                let [indexAndInterfaceIdForD] = await universalProfile1.connect(owner1).getData([tokenDMapAsset]);
                let [indexAndInterfaceIdForC] = await universalProfile1.connect(owner1).getData([tokenCMapAsset]);
            ...
            it("Should burn token A and update the key in the array and remove Map and update array length", async () => {
                ...
                let [item1Address] = await universalProfile1.connect(owner1).getData([ITEMS_ARRAY_KEY.ITEM1]);
                let [indexAndInterfaceIdForC] = await universalProfile1.connect(owner1).getData([tokenCMapAsset]);
                let [indexAndInterfaceIdForA] = await universalProfile1.connect(owner1).getData([tokenAMapAsset]);
            ...
            it("Should mint token E and register the Map and the key in the Array", async () => {
                // Token const
                const tokenEMapAsset = LSP5_ASSET_MAP_HASH + LSP7tokenE.address.substr(2);
                ...
                let [arrayLength] = await universalProfile1.connect(owner1).getData([LSP5_ARRAY_KEY]);
                let [item1Address] = await universalProfile1.connect(owner1).getData([ITEMS_ARRAY_KEY.ITEM1]);
                let [indexAndInterfaceId] = await universalProfile1.connect(owner1).getData([tokenEMapAsset]);
        ...
        describe("Transferring tokens between 2 UP", () => {
            it("Should transfer 5 tokensE to UP2 and keep 5 tokensE in UP1 and register the entries in UP2 and keep them also in UP1", async () => {
                ...
                let [arrayLengthUP1] = await universalProfile1.connect(owner1).getData([LSP5_ARRAY_KEY]);
                let [item1AddressUP1] = await universalProfile1.connect(owner1).getData([ITEMS_ARRAY_KEY.ITEM1]);
                let [indexAndInterfaceIdUP1] = await universalProfile1.connect(owner1).getData([tokenEMapAsset]);
                ...
                let [arrayLengthUP2] = await universalProfile2.connect(owner2).getData([LSP5_ARRAY_KEY]);
                let [item1AddressUP2] = await universalProfile2.connect(owner2).getData([ITEMS_ARRAY_KEY.ITEM1]);
                let [indexAndInterfaceIdUP2] = await universalProfile2.connect(owner2).getData([tokenEMapAsset]);
            ...
            it("Should mint token D to UP2 and register the map and the key in the array", async () => {
                ...
                let [arrayLengthUP2] = await universalProfile2.connect(owner2).getData([LSP5_ARRAY_KEY]);
                let [item2AddressUP2] = await universalProfile2.connect(owner2).getData([ITEMS_ARRAY_KEY.ITEM2]);
                let [indexAndInterfaceIdUP2] = await universalProfile2.connect(owner2).getData([tokenDMapAsset]);
            ...
            it("Should Transfer token D from UP2 to UP1 and remove all Token D entries", async () => {
                ...
                let [arrayLengthUP1] = await universalProfile1.connect(owner1).getData([LSP5_ARRAY_KEY]);
                let [item1AddressUP1] = await universalProfile1.connect(owner1).getData([ITEMS_ARRAY_KEY.ITEM1]);
                let [indexAndInterfaceIdUP1] = await universalProfile1.connect(owner1).getData([tokenDMapAsset]);
    ...
    describe("LSP8-IdentifiableDigitalAsset", () => {
        ...
        describe("Minting and Burning Behavior", () => {
            it("Should mint tokenA and Register the Map and the Key in the Array", async () => {
                // Token const
                const tokenAMapAsset = LSP5_ASSET_MAP_HASH + LSP8tokenA.address.substr(2);
            ...
            it("Should mint TokenB and Register the Map and the Key in the Array and Update the length", async () => {
                // Token const
                const tokenBMapAsset = LSP5_ASSET_MAP_HASH + LSP8tokenB.address.substr(2);
            ...
            it("Should mint token C and Register the Map and the key in the Array and update the length", async () => {
                // Token const
                const tokenCMapAsset = LSP5_ASSET_MAP_HASH + LSP8tokenC.address.substr(2);
            ...
            it("Should mint Token D and registering the Map and the key in the Array", async () => {
                // Token const
                const tokenDMapAsset = LSP5_ASSET_MAP_HASH + LSP8tokenD.address.substr(2);
            ...
            it("Should mint token E and register the Map and the key in the Array", async () => {
                // Token const
                const tokenEMapAsset = LSP5_ASSET_MAP_HASH + LSP8tokenE.address.substr(2);
