...
describe("UniversalProfile via EIP1167 Proxy + initializer", () => {
  ...
  describe("> Account deployment", () => {
    it("Should be cheaper to deploy via proxy", async () => {
      ...
      const { gasUsed: UniversalProfileDeploymentCost } =
        await getDeploymentCost(universalProfile);

      // Deploying via Proxy

      // 1) deploy logic contract
      let upLogicAccount = await new UniversalProfileInit__factory(
        owner
      ).deploy();
    ...
    it("Should not allow to initialize twice", async () => {
      await expect(
        proxy.initialize("0xdead0dead0dead0dead0dead0dead0dead0dead0")
  ...
  describe("> ERC1271 (signatures)", () => {
    it("Can verify signature from owner", async () => {
      ...
      const result = await proxy.callStatic.isValidSignature(
        messageHash,
        signature
      );
  ...
  describe("> Testing storage", () => {
    ...
    it("Create account", async () => {
      ...
      const newProxyAddress = await deployProxy(
    ...
    it("Should `setData` in Key-Value store via proxy (item 2)", async () => {
      let keys = [
        abiCoder.encode(["bytes32"], [ethers.utils.hexZeroPad("0xcafe", 32)]),
      ];
      let values = ["0xbeef"];
  ...
  describe("> Interactions with Accounts contracts", () => {
    ...
    it("Refuse upgrades from non-onwer", async () => {
      await expect(
        proxy.connect(newOwner).transferOwnership(newOwner.address)
    ...
    it("Fails when non-owner sets data", async () => {
      ...
      await expect(
        proxy.connect(newOwner).setData(keys, data)
      ).toBeRevertedWith("Ownable: caller is not the owner");
    });
      ...
      let keys = [
        ...
        abiCoder.encode(
          ...
          [
            ethers.utils.hexZeroPad(
              ethers.utils.hexlify(
                ethers.utils.toUtf8Bytes("Another important data")
              ),
              32
            ),
          ]
        ...
        abiCoder.encode(
          ...
          [
            ethers.utils.hexZeroPad(
              ethers.utils.hexlify(
                ethers.utils.toUtf8Bytes("more important data")
              ),
              32
            ),
          ]
  ...
  describe("> Universal Receiver", () => {
    it("Call account and check for 'UniversalReceiver' event", async () => {
      ...
      const proxyAccount = await attachUniversalProfileProxy(
        owner,
        proxyAddress
      );
    ...
    it("Mint ERC777 and LSP4 to LSP3 account", async () => {
      ...
      await erc777.mint(proxyAccount.address, "50", {
      ...
      await digitalCertificate.mint(proxyAccount.address, "50", {
        from: tokenOwner.address,
      });
    ...
    it("Transfer ERC777 and LSP4 to LSP3 account", async () => {
      ...
      let initialdigitalCertificateBalance =
        await digitalCertificate.callStatic.balanceOf(proxyAccount.address);
      ...
      await digitalCertificate
        .connect(tokenOwner)
        .send(proxyAccount.address, "50", "0x");
      await digitalCertificate
        .connect(tokenOwner)
        .transfer(proxyAccount.address, "50");

      let newERC777Balance = await erc777.callStatic.balanceOf(
        proxyAccount.address
      );
    ...
    it("Mint ERC777 and LSP4 to LSP3 account and delegate to UniversalReceiverAddressStore", async () => {
      ...
      expect(await digitalCertificate.balanceOf(proxyAccount.address)).toEqBN(
        0
      );
