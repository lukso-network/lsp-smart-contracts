...
describe("Receivers", () => {
  ...
  it("Can check for implementing interface", async () => {
    ...
    let result = await uni.callStatic.universalReceiver(
  ...
  it("Contract can check for implementing interface with Bytes32", async () => {
    let checker: UniversalReceiverTester =
      await new UniversalReceiverTester__factory(signer).deploy();
  ...
  it("Use delegate and test if it can store addresses", async () => {
    ...
    let account = await new UniversalProfile__factory(signer).deploy(
      signer.address
    );
    let checker = await new UniversalReceiverTester__factory(signer).deploy();
    ...
    let delegate: UniversalReceiverAddressStore =
      await new UniversalReceiverAddressStore__factory(signer).deploy(
        account.address
      );
    ...
    expect(
      await delegate.callStatic.containsAddress(checker.address)
    ).toBeTruthy();
    expect(
      await delegate.callStatic.containsAddress(checker2.address)
    ).toBeTruthy();
    expect(
      await delegate.callStatic.containsAddress(checker3.address)
    ).toBeTruthy();
    expect(
      await (await delegate.callStatic.getIndex(checker2.address)).toNumber()
    ).toEqual(1);
  });
});
