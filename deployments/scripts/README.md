...
1. **Deploy the raw creation bytecode** through the
...
2. **Verify the contracts on block explorers using the solc standard JSON input**, so the explorer shows the
...
Install [Foundry](https://getfoundry.sh) (`forge`, `cast`).

All `forge` commands use the dedicated profile (solc 0.8.17, optimizer 1000 runs, matching the v0.14.0 production builds).

1. Export it once per shell:

```bash
export FOUNDRY_PROFILE=deployments
```

2. Set the target chain's RPC and your funded deployer key:

```bash
export RPC_URL="https://rpc.of-the-target-chain"
export DEPLOYER_PK="0x..."         # or use --account <keystore-name> instead
```
