...
const { bytecode: AccountBytecode } = Account__factory;
const { bytecode: NonPayableConstructorBytecode } = NonPayableContract__factory;
const { bytecode: ImplementationTesterBytecode } = ImplementationTester__factory;
const { bytecode: FallbackInitializerBytecode } = FallbackInitializer__factory;
const { bytecode: PayableContractBytecode } = PayableContract__factory;

type UniversalFactoryTestAccounts = {
  random: HardhatEthersSigner;
  deployer1: HardhatEthersSigner;
  deployer2: HardhatEthersSigner;
  deployer3: HardhatEthersSigner;
  deployer4: HardhatEthersSigner;
};
  ...
  describe('When using LSP16UniversalFactory', () => {
    ...
    describe('when using deployCreate2', () => {
      ...
      it('should revert when deploying a non-initializable contract with the same bytecode and salt ', async () => {
        ...
        await expect(context.universalFactory.deployCreate2(UPBytecode, salt)).rejectedWith(
      ...
      it('should pass when sending value while deploying a payable non-initializable contract', async () => {
        ...
        await context.universalFactory.deployCreate2(PayableContractBytecode, salt, {
          value: valueSent,
        });
    ...
    describe('when testing edge cases', () => {
      describe('When deploying initializable minimal proxies via deployERC1167ProxyAndInitialize', () => {
        let salt: string;
        let initializeCallData: string;
        let contractCreatedWithdeployERC1167ProxyAndInitialize: string;
