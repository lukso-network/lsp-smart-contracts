...
describe('Gas Comparison: LSP7MintableInit vs CustomizableTokenInit', () => {
  ...
  describe('Deployment and Initialization', () => {
    it('should deploy both contracts as minimal proxies and compare gas costs', async () => {
      ...
      const customizableTokenInitBase = await new LSP7CustomizableTokenInit__factory(
        owner,
      ).deploy();
      ...
      const customizableTokenInit: CustomizableTokenInit =
        LSP7CustomizableTokenInit__factory.connect(customizableTokenProxyAddress, owner);
      ...
      const lsp7MintTx = await lsp7MintableInit.mint(
        owner.address,
        INITIAL_MINT_AMOUNT,
        true,
        '0x',
      ...
      console.log('\n=== Summary ===');
      console.log(
        'Total initialization gas difference:',
        (customizableInitGasUsed - lsp7InitGasUsed).toString(),
      );
      console.log(
        'Total mint gas difference:',
        (customizableMintGasUsed - lsp7MintGasUsed).toString(),
      );
      console.log(
        'Total transfer gas difference:',
        (customizableTransferGasUsed - lsp7TransferGasUsed).toString(),
      );
    });
  });
