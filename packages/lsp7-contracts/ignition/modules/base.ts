import { buildModule } from '@nomicfoundation/hardhat-ignition/modules';
import LSP7MintableInitModule from './LSP7MintableInit.js';

/**
 * Module that deploys all LSP7 base contracts for proxies
 *
 * This module deploys contracts that are used as implementation/base contracts
 * for proxy patterns (ERC1167 minimal proxies, etc.)
 *
 * ## Included Contracts
 * - LSP7MintableInit
 *
 * @example
 * ```bash
 * # Deploy base contracts
 * npx hardhat ignition deploy ignition/modules/base.ts
 *
 * # Deploy with CREATE2 for deterministic addresses
 * npx hardhat ignition deploy ignition/modules/base.ts --strategy create2
 * ```
 */
const LSP7BaseModule = buildModule('LSP7Base', (m) => {
  const { lsp7MintableInit } = m.useModule(LSP7MintableInitModule);

  return {
    lsp7MintableInit,
  };
});

export default LSP7BaseModule;
