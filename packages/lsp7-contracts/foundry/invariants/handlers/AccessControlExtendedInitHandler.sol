...
/// @dev Handler for AccessControlExtendedInitAbstract invariants 8–11.
contract AccessControlExtendedInitHandler is Test {
    uint256 internal constant NUM_ACTORS = 6;

    MockAccessControlExtendedInit public implementation;
    MockAccessControlExtendedInit public token;
    ...
    constructor() {
        for (uint256 i; i < NUM_ACTORS; ++i) {
            address actor = makeAddr(string(abi.encodePacked("acInitActor", i)));
            actors.push(actor);
            _trackAccount(actor);
        }
        ...
        token = MockAccessControlExtendedInit(payable(instance));
