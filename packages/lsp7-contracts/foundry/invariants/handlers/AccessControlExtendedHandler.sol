...
import {MockAccessControlExtended} from "../mocks/InvariantTestMocks.sol";
import {
    _LSP4_TOKEN_TYPE_TOKEN
} from "@lukso/lsp4-contracts/contracts/LSP4Constants.sol";

/// @dev Handler for AccessControlExtendedAbstract invariants 8–11.
contract AccessControlExtendedHandler is Test {
    uint256 internal constant NUM_ACTORS = 6;

    MockAccessControlExtended public token;
    ...
    constructor() {
        for (uint256 i; i < NUM_ACTORS; ++i) {
            address actor = makeAddr(string(abi.encodePacked("acActor", i)));
            actors.push(actor);
            _trackAccount(actor);
        }
