...
/// @dev Handler for LSP7NonTransferableInitAbstract invariants 20–23.
contract LSP7NonTransferableInitHandler is Test {
    uint256 internal constant NUM_ACTORS = 4;

    MockLSP7NonTransferableInit public implementation;
    MockLSP7NonTransferableInit public token;
    ...
    constructor() {
        for (uint256 i; i < NUM_ACTORS; ++i) {
            actors.push(makeAddr(string(abi.encodePacked("ntInitActor", i))));
        }

        implementation = new MockLSP7NonTransferableInit();
        address instance = Clones.clone(address(implementation));
        token = MockLSP7NonTransferableInit(payable(instance));

        token.initialize(
            "NonTransferable Init",
            "NTI",
            address(this),
            _LSP4_TOKEN_TYPE_TOKEN,
            false,
    ...
    function transferOwnership(uint256 newOwnerSeed) external {
        address newOwner = makeAddr(
            string(abi.encodePacked("ntInitOwner", newOwnerSeed))
        );
