...
interface ILSP7RoleOperators {
    ...
    /// @dev Can only be called by the contract owner. Does not modify any existing operator data.
    ...
    function authorizeRoleOperator(bytes32 role, address operator) external;
    ...
    function getRoleOperatorsCount(
        bytes32 role
    ...
    /// @custom:info To get all operators, call getRoleOperatorsCount() to get the total count, then call getRoleOperatorsByIndex(role, 0, count).
    /// @param role The role identifier to query.
    ...
    /// @notice Authorizes multiple operators for roles with associated data in a single transaction.
    /// @dev Can only be called by the contract owner. The roles, operators and data arrays must have the same length.
    /// @param roles The roles attributed to each authorized operators.
    ...
    /// @notice Revokes multiple operators from roles in a single transaction.
    /// @dev Can only be called by the contract owner. Cannot revoke reserved addresses (address(0), dead address).
    /// @param roles The roles to revoke each operator from.
