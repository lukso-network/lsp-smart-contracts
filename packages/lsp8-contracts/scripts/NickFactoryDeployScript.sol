...
abstract contract NickFactoryDeployScript is Script {
    ...
    function _deployViaNickFactory(bytes memory creationCode)
        internal
        returns (address deployed)
    {
        deployed = computeCreate2Address(
            IMPLEMENTATION_SALT, keccak256(creationCode), NICK_FACTORY
        );
        ...
        bytes memory deploymentData =
            abi.encodePacked(IMPLEMENTATION_SALT, creationCode);

        (bool success,) = NICK_FACTORY.call(deploymentData);
        require(success, "NickFactory: deployment failed");
        require(deployed.code.length > 0, "NickFactory: no code at predicted address");
    }
}
