...
/// @dev Handler for AccessControlExtendedInitAbstract invariants 36–39.
contract AccessControlExtendedInitHandler is Test {
    uint256 internal constant NUM_ACTORS = 6;

    MockLSP8AccessControlExtendedInit public implementation;
    MockLSP8AccessControlExtendedInit public token;
    ...
    constructor() {
        for (uint256 i; i < NUM_ACTORS; ++i) {
            address actor = makeAddr(string(abi.encodePacked("lsp8AcInitActor", i)));
            actors.push(actor);
            _trackAccount(actor);
        }

        address deployer = address(this);
        implementation = new MockLSP8AccessControlExtendedInit();
        address instance = Clones.clone(address(implementation));
        token = MockLSP8AccessControlExtendedInit(payable(instance));
