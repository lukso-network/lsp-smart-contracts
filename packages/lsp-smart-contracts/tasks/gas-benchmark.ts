...
interface GasBenchmarkTaskArguments {
  compare: string;
  against: string;
}

// eslint-disable-next-line @typescript-eslint/no-explicit-any
export default async function (taskArguments: GasBenchmarkTaskArguments): Promise<any> {
  const currentBenchmark = JSON.parse(fs.readFileSync(taskArguments.compare, 'utf8'));
  const baseBenchmark = JSON.parse(fs.readFileSync(taskArguments.against, 'utf8'));
  ...
  // ansi color codes
  console.log('\x1b[38;5;172mGenerating gas benchmark report for 🆙 + LSP Token transfers 🪙🖼️...');
...
${generatedKeyManagerSetDataTable}
  ...
  console.log('\x1b[0;32mGas benchmark report generated in `gas_benchmark.md` ✅\x1b[0m');
  return null;
}
