...
[`LSP7CappedSupplyInit.sol`](https://github.com/lukso-network/lsp-smart-contracts/tree/develop/packages/lsp7-contracts/contracts/presets/LSP7CappedSupplyInit.sol)

:::

> Proxy Implementation of the LSP7 Digital Asset standard, a contract that represents a fungible token.

LSP7 token extension to add a max token supply cap (proxy version).
...
- Solidity implementation: [`LSP7CappedSupplyInit.sol`](https://github.com/lukso-network/lsp-smart-contracts/tree/develop/packages/lsp7-contracts/contracts/presets/LSP7CappedSupplyInit.sol)
...
_Deploying a `LSP7CappedSupplyInit` token contract._
...
Same as [`_mint`](#_mint) but allows to mint only if the [`totalSupply`](#totalsupply) does not exceed the [`tokenSupplyCap`](#tokensupplycap) after `amount` of tokens have been minted.
