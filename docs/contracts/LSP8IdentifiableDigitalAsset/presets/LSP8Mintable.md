...
```solidity
constructor(string name_, string symbol_, address newOwner_, uint256 lsp4TokenType_, uint256 lsp8TokenIdFormat_);
```


*Deploying a `LSP8Mintable` token contract with: token name = `name_`, token symbol = `symbol_`, and address `newOwner_` as the token contract owner.*
...
```solidity
function authorizeOperator(address operator, bytes32 tokenId, bytes operatorNotificationData) external nonpayable;
```
...
```solidity
function getDataBatchForTokenIds(bytes32[] tokenIds, bytes32[] dataKeys) external view returns (bytes[] dataValues);
```
...
```solidity
function getDataForTokenId(bytes32 tokenId, bytes32 dataKey) external view returns (bytes dataValue);
```
...
```solidity
function revokeOperator(address operator, bytes32 tokenId, bool notify, bytes operatorNotificationData) external nonpayable;
```
...
```solidity
function setDataBatchForTokenIds(bytes32[] tokenIds, bytes32[] dataKeys, bytes[] dataValues) external nonpayable;
```
...
```solidity
function setDataForTokenId(bytes32 tokenId, bytes32 dataKey, bytes dataValue) external nonpayable;
```
...
```solidity
function _isOperatorOrOwner(address caller, bytes32 tokenId) internal view returns (bool);
```
...
```solidity
function _revokeOperator(address operator, address tokenOwner, bytes32 tokenId, bool notified, bytes operatorNotificationData) internal nonpayable;
```
...
```solidity
function _clearOperators(address tokenOwner, bytes32 tokenId) internal nonpayable;
```
...
```solidity
function _mint(address to, bytes32 tokenId, bool force, bytes data) internal nonpayable;
```
...
```solidity
function _transfer(address from, address to, bytes32 tokenId, bool force, bytes data) internal nonpayable;
```
...
```solidity
function _setDataForTokenId(bytes32 tokenId, bytes32 dataKey, bytes dataValue) internal nonpayable;
```
...
```solidity
function _getDataForTokenId(bytes32 tokenId, bytes32 dataKey) internal view returns (bytes dataValues);
```
...
```solidity
function _beforeTokenTransfer(address from, address to, bytes32 tokenId, bytes data) internal nonpayable;
```
...
```solidity
function _afterTokenTransfer(address from, address to, bytes32 tokenId, bytes data) internal nonpayable;
```
...
```solidity
function _notifyTokenOperator(address operator, bytes lsp1Data) internal nonpayable;
```
...
```solidity
function _notifyTokenReceiver(address to, bool force, bytes lsp1Data) internal nonpayable;
```
