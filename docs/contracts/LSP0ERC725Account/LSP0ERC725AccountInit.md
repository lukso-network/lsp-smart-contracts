...
[`LSP0ERC725AccountInit.sol`](https://github.com/lukso-network/lsp-smart-contracts/tree/develop/packages/lsp0-contracts/contracts/LSP0ERC725AccountInit.sol)

:::

> Deployable Proxy Implementation of [LSP-0-ERC725Account] Standard.
...
- Solidity implementation: [`LSP0ERC725AccountInit.sol`](https://github.com/lukso-network/lsp-smart-contracts/tree/develop/packages/lsp0-contracts/contracts/LSP0ERC725AccountInit.sol)
...
_deploying a `LSP0ERC725AccountInit` base contract to be used behind proxy_

Locks the base contract on deployment, so that it cannot be initialized, owned and controlled by anyone after it has been deployed. This is intended so that the sole purpose of this contract is to be used as a base contract behind a proxy.
...
- Specification details: [**LSP-0-ERC725Account**](https://github.com/lukso-network/LIPs/blob/main/LSPs/LSP-0-ERC725Account.md#initialize)
- Solidity implementation: [`LSP0ERC725AccountInit.sol`](https://github.com/lukso-network/lsp-smart-contracts/tree/develop/packages/lsp0-contracts/contracts/LSP0ERC725AccountInit.sol)
...
_Initializing a LSP0ERC725Account contract with owner set to address `initialOwner`._

Set `initialOwner` as the contract owner. The `initialize(address)` also allows funding the contract on deployment.

<blockquote>

**Emitted events:**

- [`UniversalReceiver`](#universalreceiver) event with typeId `LSP0ValueReceived` when funding the contract on initialization.
...
ERC725X & ERC725Y parent contracts are not initialized as they don't have non-zero initial state. If you decide to add non-zero initial state to any of those contracts, you MUST initialize them here.
...
Set `initialOwner` as the contract owner. The `initialOwner` will then be allowed to call protected functions marked with the `onlyOwner` modifier.
...
- Specification details: [**LSP-0-ERC725Account**](https://github.com/lukso-network/LIPs/blob/main/LSPs/LSP-0-ERC725Account.md#initialized)
- Solidity implementation: [`LSP0ERC725AccountInit.sol`](https://github.com/lukso-network/lsp-smart-contracts/tree/develop/packages/lsp0-contracts/contracts/LSP0ERC725AccountInit.sol)
