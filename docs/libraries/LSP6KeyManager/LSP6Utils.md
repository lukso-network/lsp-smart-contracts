...
[`LSP6Utils.sol`](https://github.com/lukso-network/lsp-smart-contracts/tree/develop/packages/lsp6-contracts/contracts/LSP6Utils.sol)
...
If the raw value fetched from the ERC725Y storage of `target` is not 32 bytes long, this is considered like _"no permissions are set"_ and will return 32 x `0x00` bytes as `bytes32(0)`.
...
| Name     |    Type     | Description                                                                                                                                                                                                                          |
| -------- | :---------: | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `keys`   | `bytes32[]` | An array of 3 x data keys containing: - `keys[0] = AddressPermissions[]` (array length). - `keys[1] = AddressPermissions[index]` (where to store the controller address). - `keys[2] = AddressPermissions:Permissions:<controller>`. |
| `values` |  `bytes[]`  | An array of 3 x data values containing: - `values[0] =` the new array length of `AddressPermissions[]` - `values[1] =` the address of the controller - `values[2] =` the `permissions` passed as param                               |
